...
const Pricing = ({ onSelectPlan }) => {
    ...
    if (loading) return <div className="py-24 text-center text-lg text-(--text-secondary) animate-pulse">Loading plans...</div>;
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <div className="text-center mb-20">
                    ...
                    <p className="text-xl text-(--text-secondary) max-w-2xl mx-auto">
                        {t('landing.pricing_subtitle') || 'Start with a plan that fits your needs. Upgrade as you grow.'}
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
                    {plans.map((plan) => (
                        <div
                            key={plan.id}
                            className={`flex flex-col p-8 rounded-3xl transition-all duration-300 ${plan.name === 'Premium'
                        ...
                        >
                            ...
                            <div className="flex items-baseline mb-8">
                                <span className="text-5xl font-extrabold text-(--text-primary) tracking-tight">{plan.price}{t('landing.currency')}</span>
                                <span className="text-lg text-(--text-secondary) ml-2">/{t('common.month')}</span>
                            </div>
                                    ...
                                    {(() => {
                                        ...
                                        return features.map((feature, idx) => (
                                            <li key={idx} className="flex items-start text-(--text-secondary)">
                                                ...
                                                <span className="text-base">{t(`features.${feature.trim()}`) || feature}</span>
                                            </li>
                                        ));
                                    })()}
                                </ul>
                            ...
                            <button
                                ...
                                className={`w-full py-4 rounded-xl font-bold text-lg transition-all transform hover:scale-[1.02] ${plan.name === 'Premium'
